...
class ComDrug extends Api
{
	...
	public function index()
	{
		...
		if( !empty($ticketType) ){
			$condition[] = ['dh.two_votes','=',$ticketType];
		}
		...
		if( is_array($condition) ){
			...
			$where = $condition;
			//筛选判断
			$res = Db::table('drug_sales_enterprise')->alias($alias)->join($join)->where($where)->group('dh.drug_number')->limit(0,$pageIndex * 30)->select();
			$more =Db::table('drug_sales_enterprise')->alias($alias)->join($join)->where($where)->group('dh.drug_number')->limit($pageIndex * 30,30)->select();	//判断是否还有数据

		}elseif( !empty($condition) && !is_array($condition) ){


			$sql = "SELECT * FROM ( select * from `drug_sales_enterprise` `dh` where `dh`.`state`=1 and `dh`.`delete_flag` = 2 and `dh`.`enterprise_id` = '".$companyId."' ) as H INNER JOIN ( SELECT * FROM `drug_system` `ds` WHERE `ds`.`generic_name` LIKE '%".$condition."%' OR `ds`.`generic_name_pinyin` LIKE '%".strtoupper($condition)."%' OR `ds`.`production_enterprise` LIKE '%".$condition."%' OR `ds`.`production_enterprise_pinyin` LIKE '%".strtoupper($condition)."%') AS S ON `S`.`system_number`=`H`.`drug_number` group by `H`.drug_number  LIMIT 0,".$pageIndex * 30;

			$sqlMore = "SELECT * FROM ( select * from `drug_sales_enterprise` `dh` where `dh`.`state`=1 and  `dh`.`delete_flag` = 2 and `dh`.`enterprise_id` = '".$companyId."' ) as H INNER JOIN ( SELECT * FROM `drug_system` `ds` WHERE `ds`.`generic_name` LIKE '%".$condition."%'  OR  `ds`.`production_enterprise` LIKE '%".$condition."%' OR `ds`.`production_enterprise_pinyin` LIKE '%".strtoupper( $condition )."%') AS S ON `S`.`system_number`=`H`.`drug_number` group by `H`.drug_number LIMIT ".($pageIndex * 30)." ,30";
